...
const corsOptions = {
    origin: ['http://127.0.0.1:5500', 'http://localhost:5500', 'http://localhost:8080'], // Allow both localhost and 127.0.0.1 for frontend
    credentials: true,
...
const fs = require('fs');
function executeSchemaFromFile(db) {
    return new Promise((resolve, reject) => {
        fs.readFile('./schema.sql', 'utf8', (err, sql) => {
            ...
            // Split the SQL into statements and skip CREATE DATABASE and USE
            const statements = sql.split(';').map(stmt => stmt.trim()).filter(stmt => stmt.length > 0 && !stmt.toUpperCase().startsWith('CREATE DATABASE') && !stmt.toUpperCase().startsWith('USE'));
            // Execute each statement sequentially
            const executeNext = (index) => {
                if (index >= statements.length) {
                    console.log('Database schema created from schema.sql successfully');
                    resolve();
                    return;
                }
...
async function initDB() {
    return new Promise((resolve, reject) => {
        db.connect(err => {
            ...
            db.query('CREATE DATABASE IF NOT EXISTS subscription_management_model', (err) => {
                if (err) {
                ...
                db.query('USE subscription_management_model', (err) => {
                    if (err) {
                        console.error('Error switching to database:', err);
                        reject(err);
                        return;
                    }
                    console.log('Switched to subscription_management_model database.');

                    // Execute schema to create tables from schema.sql file
                    executeSchemaFromFile(db).then(() => {
